  ...
  return helpers.makeLiveSlots(
    ...
    (E, D) => {
      ...
      async function fillTwoPurses(moolahPurseName, simoleanPurseName) {
        const moola = await E(vats.mints).getNewPurse('moola', moolahPurseName);
        const simolean = await E(vats.mints).getNewPurse('simolean', simoleanPurseName);
      ...
      return harden({
        async bootstrap(argv, vats, devices) {
          ...
          if (ROLES.chain || ROLES.one_chain) {
            ...
            await E(vats.pixel).startup(host);
            await E(vats.timer).registerTimerDevice(devices.timer);
            const bundler = await makeBundler(host, vats).createDemoBundle('localuser');

            await E(vats.exchange).startup(host, bundler.zoe, bundler.registrar);
            await E(vats.wallet).startup(host, bundler.zoe, bundler.registrar);
